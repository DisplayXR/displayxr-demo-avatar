...
// speech-bubble band, composited flat as a Local2D layer (XR_DXR_local_3d_zone).
...
// runtime advertises XR_DXR_local_3d_zone). 2048x512 RGBA8.
...
static bool CreateMacOSWindow(uint32_t width, uint32_t height, int32_t panelLeft, int32_t panelTop) {
    ...
    // (XrDisplayDesktopPositionDXR, display_info v16); AppKit is bottom-up,
...
struct AppXrSession {
    ...
    bool hasLocal3DZone = false;   // XR_DXR_local_3d_zone — speech-bubble Local2D layer
    ...
    // primary screen's top-left (XrDisplayDesktopPositionDXR, display_info v16,
...
static bool InitializeOpenXR(AppXrSession& xr) {
    ...
    for (const auto& ext : exts) {
        ...
        if (strcmp(ext.extensionName, XR_DXR_LOCAL_3D_ZONE_EXTENSION_NAME) == 0) xr.hasLocal3DZone = true;
    }
    ...
    if (xr.hasLocal3DZone) enabled.push_back(XR_DXR_LOCAL_3D_ZONE_EXTENSION_NAME);
...
static void ApplyAutoFitForLoadedScene() {
    ...
    if (g_modelRenderer.getRobustSceneBounds(0.05f, 0.95f, center, extent)) {
        ...
        if (g_modelRenderer.getActiveClipBounds(aMin, aMax)) {
            ...
            vh = dxr::AutoFitVHeight(subjW, subjH, canvasW, canvasH);
            ...
            // canvas edge, even though the active clip decided the size.
        ...
        LOG_INFO("Auto-fit: center=(%.3f, %.3f, %.3f) extent=(%.3f, %.3f, %.3f) "
                 "vHeight=%.3f (%s box, %s-bound, canvas=%.0fx%.0f aspect=%.3f fill=%.2f) "
                 "yaw=%.0fdeg",
...
static void TryAutoLoadBundledScene(const std::string& overridePath = std::string()) {
    ...
    // active clip's own box — see ApplyAutoFitForLoadedScene). Sticky on the
    // renderer, but set at each load site so no path can drift.
...
// Set when the image is (re)created: UNDEFINED, not yet through RecordSilhouettePrep.
static bool        g_silImageFresh = false;

// avatar#99 (same fix as the Windows leg): renderEye picks its entry layout from
// firstViewInImage = (viewportX == 0 && viewportY == 0), and the silhouette
// renders into the bottom canvas band (viewportY != 0), so it always claims
// COLOR_ATTACHMENT_OPTIMAL — while the image is UNDEFINED on first use and
// TRANSFER_SRC_OPTIMAL after every readback copy. Keep the claim true: between
// renderEye calls the scratch image sits cleared in COLOR_ATTACHMENT_OPTIMAL.
...
static bool EnsureSilhouetteTargets(VkDevice dev, VkPhysicalDevice phys, uint32_t w, uint32_t h) {
    ...
    // avatar#99: COLOR_ATTACHMENT is required too — renderEye's barriers name
    // COLOR_ATTACHMENT_OPTIMAL, and modelCreateImage2D always creates a view,
    // which a TRANSFER-only image cannot have ("failed to create image view").
...
static void UpdateSilhouette(VkDevice dev, VkPhysicalDevice phys, VkQueue queue, VkCommandPool pool,
    ...
    for (uint32_t p = 0; p < silPasses; ++p) {
        ...
        // avatar#99: back to cleared COLOR_ATTACHMENT_OPTIMAL for the next renderEye.
...
// Local2D layer (XR_DXR_local_3d_zone) so the avatar keeps weaving in the
...
int main(int argc, char** argv) {
    ...
    // advertises XR_DXR_local_3d_zone (the Metal compositor does).
        ...
        if (xr.sessionRunning) {
            ...
            if (BeginFrame(xr, frameState)) {
                ...
                if (rendered) {
                    ...
                    XrCompositionLayerLocal2DDXR bubbleBgLayer = {(XrStructureType)XR_TYPE_COMPOSITION_LAYER_LOCAL_2D_DXR};
